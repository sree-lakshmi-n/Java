...
class Rainfall{
    private static double[] convertToDoubleArray(String town, String strng){
        String[] mainArr = strng.split("\n");
        ...
        for(int i = 0; i< doubleArr.length; i++){
            doubleArr[i] = Double.parseDouble(strArr[2*i+1]);
        }
        return doubleArr;
    }
    public static double mean(String town, String strng) {
        if(!strng.contains(town+":"))
            return -1.0;
        double[] townData = convertToDoubleArray(town,strng);
        double result = 0;
        for (double d: townData) {
            result += d;
        }
        result /= townData.length;
        return result;
    }
...
public class RainfallTest {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("London: \n Mean -> "+rain.mean("London", RainfallTest.data));
        System.out.println("Beijing: \n Mean -> "+rain.mean("Beijing", RainfallTest.data));
        System.out.println("London: \n Variance -> "+rain.variance("London", RainfallTest.data));
        System.out.println("Beijing: \n Variance -> "+rain.variance("Beijing", RainfallTest.data));
        System.out.println("Vancouver: \n Mean -> "+rain.mean("Vancouver", RainfallTest.data));
        for (String s : shuffle(RainfallTest.towns))
            System.out.println(s+ ": \n Mean -> "+rain.mean(s, RainfallTest.data));
        for (String s : shuffle(RainfallTest.towns))
            System.out.println(s+ ": \n Variance -> "+rain.variance(s, RainfallTest.data));
        for (String s : shuffle(RainfallTest.towns))
            System.out.println(s+ ": \n Mean -> "+rain.mean(s, RainfallTest.data1));
        for (String s : shuffle(RainfallTest.towns))
            System.out.println(s+ ": \n Variance -> "+rain.variance(s, RainfallTest.data1));
    }
}
